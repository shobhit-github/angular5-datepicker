...
import {Component, ComponentRef, ElementRef, Input, Output, OnInit, AfterViewInit, ViewChild, EventEmitter} from '@angular/core';
...
@Component({
  ...
  styles: [`
    .datepicker-actions__button, .datepicker-actions__input {
    ...
    .datepicker-actions__button:disabled, .datepicker-actions__input:disabled, .datepicker-actions__input[readonly] {
      opacity: 1;
      background-color: #eee
    }

    .datepicker-actions__button:disabled, .datepicker-actions__input:disabled {
      cursor: not-allowed
    }
    ...
    .datepicker-actions__button:active, .datepicker-actions__button:focus {
      border-color: #4d90fe;
      box-shadow: inset 0 1px 1px rgba(0, 0, 0, .075), 0 0 8px rgba(102, 175, 233, .6)
    }
...
export class DatePicker implements ControlValueAccessor, OnInit, AfterViewInit {
  ...
  @Input() public mode: 'date' | 'datetime' | 'time' = 'date';
  @Input() public displayDateMode: 'day' | 'month' | 'year' = 'day';
  @Input() public showClearButton: boolean = true;
  @Input() public format: string;
  @Input() public isMeridiem: boolean = true;
  @Input() public disabled: boolean;
  @Input() public align: any;
  @Input() public closeBySelection: boolean = false;
  @Input() public inputClass: string;
  @Input() public initialDate: Date;

  @Output() public dateUpdated: EventEmitter<any> = new EventEmitter();
  ...
  public raiseOnChange(value: string): void {
    ...
    if (!parsed) {
      ...
      this.dateUpdated.emit({ date: formatted });
    } else {
  ...
  public openPopup(): void {
    ...
    this.overlayService.openComponentInPopup<DatePickerPanel>(
      ...
      c.instance.initialDate = this.initialDate;
