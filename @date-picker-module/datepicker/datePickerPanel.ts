...
@Component({
    ...
    `,
    styles: [
        `.datepicker-panel{display:flex;overflow:hidden;max-width:17em;margin-top:1em;padding:0;border:1px solid #ccc;background:#fff;flex-flow:column nowrap;justify-content:center;align-items:center}`
    ]
})
export class DatePickerPanel implements ControlValueAccessor {

    @Input('type') public mode: 'date' | 'datetime' | 'time' = 'date';
    @Input('displayDateMode') public displayDateMode: 'day' | 'month' | 'year' = 'day';
    @Input() public isMeridiem: boolean = true;
    @Input() public initialDate: Date;

    @Output() public dateChange: EventEmitter<Moment> = new EventEmitter<Moment>();
    @Output() public dateSelected: EventEmitter<Moment> = new EventEmitter<Moment>();
    @Output() public modeChanged: EventEmitter<any> = new EventEmitter<any>();
    ...
    public writeValue(value: any): void {
        let parsedValue = local(value || this.initialDate);
